...
const casesPath = path.join(__dirname, '..', '..', 'case-tests', 'tracklist.cases.json');
const raw = fs.readFileSync(casesPath, 'utf8');
const payload = JSON.parse(raw);
const cases = payload.cases || [];
const docPath = payload.document || 'case-tests/tracklist.md';
...
const printHeader = () => {
  console.log('Tracklist Test Runner');
