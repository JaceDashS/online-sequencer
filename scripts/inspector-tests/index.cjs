...
const casesPath = path.join(__dirname, '..', '..', 'case-tests', 'inspector.cases.json');
const raw = fs.readFileSync(casesPath, 'utf8');
const payload = JSON.parse(raw);
const cases = payload.cases || [];
const docPath = payload.document || 'case-tests/inspector.md';
...
const printHeader = () => {
  console.log('Inspector Test Runner');
