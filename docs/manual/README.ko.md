...
- **가시 구간 렌더링**: 화면에 보이는 영역과 오버스캔 영역만 렌더링하여 성능을 향상시켰습니다
- **가상 스크롤**: 대용량 프로젝트에서도 부드러운 스크롤을 위해 가상 스크롤 방식을 사용합니다
- **플레이헤드 최적화**: 재생 헤드는 별도의 컴포넌트로 분리되어 React 리렌더링 없이 업데이트됩니다

### 오디오 샘플 로딩 최적화

- **지연 로딩**: 필요한 샘플만 로드하여 초기 로딩 시간을 단축했습니다
- **중복 로딩 방지**: 동일한 샘플의 중복 로딩을 방지합니다
- **프리페치**: 재생 시작 전 필요한 샘플을 미리 로드합니다

### MIDI 에디터 성능 개선

- **플레이헤드 분리**: 재생 헤드를 별도 컴포넌트로 분리하여 성능을 개선했습니다
- **메모이제이션**: 자주 계산되는 값들을 메모이제이션하여 불필요한 재계산을 방지합니다
